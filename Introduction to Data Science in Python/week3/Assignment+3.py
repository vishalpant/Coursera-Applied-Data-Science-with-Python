...
def trimdata(data):
    d = "".join([i for i in data if not i.isdigit()])
    if d.find('(') > -1:
        d = d[:d.find('(')]
    return d.strip()
...
energy['Country'] = energy['Country'].apply(trimdata)
...
def answer_three():
    Top15 = answer_one()
    years = Top15[['2006', '2007', '2008','2009','2010','2011','2012','2013','2014','2015']]
    return (years.mean(axis=1)).sort_values(ascending=False).rename('avgGDP')
answer_three()
...
def answer_six():
    Top15 = answer_one()
    Top15.sort_values('% Renewable', inplace=True, ascending=False)
    return (Top15.iloc[0].name, Top15.iloc[0]['% Renewable'])
answer_six()
...
def answer_seven():
    ...
    Top15.sort_values('ratio of Self-Citations to Total Citations', ascending=False, inplace=True)
    return Top15.iloc[0].name, Top15.iloc[0]['ratio of Self-Citations to Total Citations']
answer_seven()
...
def answer_eight():
    ...
    return (Top15.sort_values('Population', ascending =False)).iloc[2].name
answer_eight()
